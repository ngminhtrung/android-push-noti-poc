...
```mermaid
flowchart TD
    A[Launch Android app] --> B{Notification permission granted?}
    B -->|No| C[Request POST_NOTIFICATIONS permission\n(Android 13+)]
    C -->|Denied| D[Show rationale Toast/dialog\nApp stays limited]
    C -->|Granted| E[Continue startup]
    B -->|Yes| E

    E --> F[FirebaseMessaging.getInstance().token]
    F --> G{Token fetch success?}
    G -->|No| H[Show error text + log failure]
    G -->|Yes| I[Display token in UI & save via MessageStore]

    I --> J[User taps “Fetch Token”]\nJ --> F

    I --> K[Device ready for FCM]

    subgraph Messaging Service
        L[FCM push arrives\n(notification or data-only)]
        L --> M[PushMessagingService.onMessageReceived]
        M --> N[Append payload to MessageStore]
        M --> O[Broadcast update via LocalBroadcastManager]
        M --> P[Show toast + notification]
        O --> Q[MainActivity updates message log]
    end

    subgraph Python Sender
        R[Update sender-config.json\n(service account, project id, token…)]
        R --> S[python tools/sender/send_fcm.py]
        S --> T{OAuth token OK?}
        T -->|No| U[Print auth error]
        T -->|Yes| V[POST HTTP v1 request to FCM]
        V --> W{HTTP response}
        W -->|Success| L
        W -->|Error| X[Print diagnostics\n(e.g. 403, invalid token)]
    end
```